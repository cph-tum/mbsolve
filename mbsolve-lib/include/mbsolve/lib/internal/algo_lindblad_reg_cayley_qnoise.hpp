...
public:
    ...
    static Eigen::Rand::Vmt19937_64 generator;
    ...
    static inline qm_operator convert_density(const density& d)
    {
        ...
        for (int i = 0, row = 0, col = 1;
             (row < num_lvl) && (col < num_lvl) && (i < num_off);
             i++) {
        ...
        real time_step)
    {
        ...
        if (qm) {
             ...
             * the population scattering rates
            ...
            sc.F1 = real_matrix_f::Zero();
            for (int i = 0, row = 0, col = 1;
                 (row < num_lvl) && (col < num_lvl) && (i < deph.size());
                 i++) {
                sc.F2.row(i) = sc.d1.row(col);
                sc.F2(i, col) -= 2 * deph[i];
                sc.F1(i, col) = sc.d1(row,col);
                sc.F1(i, row) = sc.d1(col,row);
                if (row == col - 1) {
                    row = 0;
                    col++;
                } else {
...
private:
        ...
        const density& d)
    {
        ...
        complex_vector_f F_diag = (sc.F1 * d.diagonal());
        /* random number generator*/
        Eigen::MatrixXd v{3, num_lvl * (num_lvl - 1) / 2};
        v = Eigen::Rand::normalLike(v, generator);
        for (int i = 0, row = 0, col = 1; (row < num_lvl) &&
             (col < num_lvl) && (i < (num_lvl * (num_lvl - 1) / 2));
             i++) {
            // add noise terms to off-diagonals
            real xi1 = v(0, i);
            real xi2 = v(1, i);
            std::complex<real> xi = std::complex<real>(xi1, xi2);
                // Discretization of noise terms
            d1(row, col) = xi *
                std::sqrt(F_coh(i) * sc.dt /
                        (2 * sc.carrier_gridcell));
            d1(col, row) = std::conj(xi) *
                std::sqrt(F_coh(i) * sc.dt /
                          (2 * sc.carrier_gridcell));
            // add noise terms to diagonals
            d1(row,row) += v(2, i) *  std::sqrt(F_diag(i) * sc.dt /
                            (sc.carrier_gridcell));
            d1(col,col) -= d1(row,row);
            if (row == col - 1) {
                row = 0;
                col++;
            } else {
...
template<unsigned int num_lvl>
Eigen::Rand::Vmt19937_64 lindblad_reg_cayley_qnoise<num_lvl>::generator{
